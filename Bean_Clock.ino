 ...
 ***********************************************************
 * May 20, 2015 modified by kodera2t for LightBlue Bean..
 * Still on-going code and hardly completed. and scribble level.
 * No warranty at all
 ***************************************************************/
...
// SSD1306 Definition //
//////////////////////////
#define PIN_RESET 0  // SDA is connected to A0 on Bean
...
void initClockVariables()
{
  ...
  POS_12_X = MIDDLE_X - oled.getFontWidth()-xoffset;
  POS_12_Y = MIDDLE_Y - CLOCK_RADIUS + 2;
  POS_3_X  = MIDDLE_X + CLOCK_RADIUS - oled.getFontWidth() - 1-xoffset;
  POS_3_Y  = MIDDLE_Y - oled.getFontHeight()/2;
  POS_6_X  = MIDDLE_X - oled.getFontWidth()/2-xoffset;
  POS_6_Y  = MIDDLE_Y + CLOCK_RADIUS - oled.getFontHeight() - 1;
  POS_9_X  = MIDDLE_X - CLOCK_RADIUS + oled.getFontWidth() - 2-xoffset;
...
void drawArms(int h, int m, int s)
{
  ...
  oled.line(MIDDLE_X-xoffset, MIDDLE_Y, MIDDLE_X + sx-xoffset, MIDDLE_Y + sy);
  
  m = map(m, 0, 60, 0, 360);  // map the 0-60, to "360 degrees"
  mx = M_LENGTH * cos(PI * ((float)m) / 180);  // woo trig!
  my = M_LENGTH * sin(PI * ((float)m) / 180);  // woo trig!
  // draw the minute hand
  oled.line(MIDDLE_X-xoffset, MIDDLE_Y, MIDDLE_X + mx-xoffset, MIDDLE_Y + my);
  ...
  // draw the hour hand:
  oled.line(MIDDLE_X-xoffset, MIDDLE_Y, MIDDLE_X + hx-xoffset, MIDDLE_Y + hy);
}
...
void drawFace()
{
  ...
  oled.circle(MIDDLE_X-xoffset, MIDDLE_Y, CLOCK_RADIUS);
  ...
  oled.print(3);
  oled.setCursor(MIDDLE_X+sep,POS_12_Y);
  oled.print("New Mail!");
  oled.setCursor(MIDDLE_X+sep,POS_12_Y+10);
  oled.print("May 20");
  oled.setCursor(MIDDLE_X+sep,POS_12_Y+20);  
  oled.print("2015");
  oled.setCursor(MIDDLE_X+sep,POS_12_Y+30); 
  int temperature = Bean.getTemperature();
  oled.print(temperature);
  oled.setCursor(MIDDLE_X+sep+15,POS_12_Y+30);
    oled.print("deg");
 uint16_t batteryReading =  Bean.getBatteryVoltage(); 
 
 // Format the output like "Battery voltage: 2.60 V"
 String stringToPrint = String();
 stringToPrint = stringToPrint + batteryReading/100 + "." + batteryReading%100 + " V";    
  oled.setCursor(MIDDLE_X+sep,POS_12_Y+40);  
  oled.print(stringToPrint);
//  oled.display();
}
